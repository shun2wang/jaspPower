...
.runZtestOneS <- function(jaspResults, options) {
  stats <- .prepareStats(options)

  ## Compute results
  results <- try(.computeZtestOneS(jaspResults, options, stats))
  .checkResults(results)

  .initPowerTabZtestOneS(jaspResults, options, results, stats)

  if (options$text) {
    .initPowerESTabZtestOneS(jaspResults, options, results, stats)
  }

  ## Populate tables and plots.populateIntro(jaspResults, options)

  if (options$powerContour) {
    .preparePowerContourZtestOneS(jaspResults, options, results, stats)
    if (options$text) {
      .populateContourTextZtestOneS(jaspResults, options, results, stats)
    }
  }
  if (options$powerByEffectSize) {
    .preparePowerCurveESZtestOneS(jaspResults, options, results, stats)
    if (options$text) {
      .populatePowerCurveESTextZtestOneS(jaspResults, options, results, stats)
    }
  }
  if (options$powerBySampleSize) {
    .preparePowerCurveNZtestOneS(jaspResults, options, results, stats)
    if (options$text) {
      .populatePowerCurveNTextZtestOneS(jaspResults, options, results, stats)
    }
  }
  if (options$powerDemonstration) {
    .preparePowerDistZtestOneS(jaspResults, options, results, stats)
    if (options$text) {
      .populateDistTextZtestOneS(jaspResults, options, results, stats)
    }
  }
  if (options$saveDataset && options$savePath != "") {
    .generateDatasetZtestOneS(jaspResults, options, results, stats)
  }
}

#### Compute results ----
.computeZtestOneS <- function(jaspResults, options, stats) {
...
#### Init table ----
.initPowerTabZtestOneS <- function(jaspResults, options, results, stats) {
  ...
  .populatePowerTabZtestOneS(jaspResults, options, results, stats)
}
.initPowerESTabZtestOneS <- function(jaspResults, options, results, stats) {
  ...
  .populatePowerESTabZtestOneS(jaspResults, options, results, stats)
}
...
.preparePowerCurveESZtestOneS <- function(jaspResults, options, r, lst) {
  ...
  maxd <- try(pwr::pwr.norm.test(n = n, power = max(0.999, power), sig.level = alpha, alternative = alt)$d)
